...
const FichaResumo = ({ patient }: { patient: fhir.Patient }) => {
  ...
  const launchWorkspace =
    useLaunchWorkspaceRequiringVisit(fichaResumoWorkspace);
  ...
  if (fichaResumo.preTarvBookNumber) {
    preTarvText = `Nr. ${+fichaResumo.preTarvBookNumber
      .value}, Página ${+fichaResumo.preTarvBookPage
      .value}, Linha ${+fichaResumo.preTarvBookLine.value}`;
  }

  let tarvText = "N/A";
  if (fichaResumo.tarvBookNumber) {
    tarvText = `Nr. ${+fichaResumo.tarvBookNumber.value}, Página ${+fichaResumo
      .tarvBookPage.value}, Linha ${+fichaResumo.tarvBookLine.value}`;
  }
  ...
  return (
    <div className={styles.widgetCard}>
      ...
      <div className={styles.cardBody}>
        ...
        <div>
          <span className={styles.label}>
            {t("openingDate", "Data de Abertura")}
          </span>
          <div>
            {formatDatetime(
        ...
        {/* Confident Section */}
        <h4 className={styles.cardSectionHeader}>
          {t("confidant", "Confidente")}
        </h4>
        <div className={styles.cardSection}>
          <div>
          ...
          <div>
            <span className={styles.label}>
              {t("relationship", "Parentesco")}
            </span>
            <div>
              {(fichaResumo.confidantRelationship?.value as Concept)?.display}
            </div>
          </div>
          <div>
            <span className={styles.label}>
              {t("phone1", "Telefone Celular (1)")}
            </span>
            <div>{fichaResumo.confidantPhone1?.value.toString()}</div>
          </div>
          <div>
            <span className={styles.label}>
              {t("phone2", "Telefone Celular (2)")}
            </span>
        ...
        {/* Confident Section */}
        <h4 className={styles.cardSectionHeader}>
          {t("familyStatus", "Situação da família")}
        </h4>
        <StructuredListWrapper>
          <StructuredListHead>
            <StructuredListRow head>
              ...
              <StructuredListCell head>
                {t("hivTest", "Teste de HIV")}
              </StructuredListCell>
              <StructuredListCell head>
                {t("hivCare", "Cuidados de HIV")}
              </StructuredListCell>
              <StructuredListCell head>
                {t("inCCR", "Em CCR")}
              </StructuredListCell>
          ...
          <StructuredListBody>
            {fichaResumo.familyStatus.map((family, i) => (
              <StructuredListRow>
                <StructuredListCell>
                  {family.relativeName?.value.toString()}
                </StructuredListCell>
                <StructuredListCell>
                ...
                <StructuredListCell>
                  {family.age?.value.toString()}
                </StructuredListCell>
                <StructuredListCell>
                  {(family.hivTest?.value as Concept)?.display}
                </StructuredListCell>
                <StructuredListCell>
                  {(family.hivCare?.value as Concept)?.display}
                </StructuredListCell>
                <StructuredListCell>
                  {(family.ccr?.value as Concept)?.display}
                </StructuredListCell>
                <StructuredListCell>
                  {family.relativeNid?.value.toString()}
                </StructuredListCell>
              </StructuredListRow>
            ))}
          </StructuredListBody>
